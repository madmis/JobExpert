...
$arrNamePage = array(
	...
	array('name' => MENU_AGENCIES, 'link' => false)
);
...
if ($arrActions['config']) {
	...
	if (isset($_POST['save'])) {
		$data = "<?php\n\n"
				. "(!defined('SDG')) ? die ('Triple protection!') : null;\n\n"
				. 'define("CONF_AGENCIES_STRINGS_PERPAGE_ADMIN_PANEL", "' . ((int) $_POST['admperpage'] ? (int) abs($_POST['admperpage']) : 30) . '");' . "\n\n"
				. 'define("CONF_AGENCIES_PERPAGE", "' . ((int) $_POST['perpage'] ? (int) abs($_POST['perpage']) : 30) . '");' . "\n\n"
				. 'define("CONF_AGENCIES_SHOW_ONLY_WITH_LOGO", "' . ((!isset($_POST['with_logo'])) ? false : true) . '");' . "\n\n"
				. 'define("CONF_AGENCIES_DELETE_LOGO", "' . ((!isset($_POST['delete_logo'])) ? false : true) . '");' . "\n\n"
				. 'define("CONF_AGENCIES_USE_VISUAL_EDITOR", "' . ((!isset($_POST['html'])) ? false : true) . '");' . "\n\n"
				. 'define("CONF_AGENCIES_SHOW_MAIN_LOGO", "' . ((!isset($_POST['show_main_logo'])) ? false : true) . '");' . "\n\n"
				. 'define("CONF_AGENCIES_SHOW_MAIN_LOGO_QTY", "' . ((!empty($_POST['logo_qty']) && (int) $_POST['logo_qty']) ? (int) abs($_POST['logo_qty']) : 5) . '");' . "\n";

		// чистим кеш
		//caching::clearCache('company.logo');

		if (!tools::saveConfig('core/conf/const.config.agencies.php', $data, CONF_ADMIN_FILE . '?m=users&s=agencies&action=config')) {
	...
	$seoUrl = '?m=users&s=agencies&action=seo';
	...
	$smarty->assign('seo', seo::getSeoFiles('agencies', $currLang));
} else {
	/**
	 * Действия
	 */
	if (!empty($_POST['action'])) {
		...
		if ('show' === $_POST['action'] && !empty($_POST['agencies'])) {
			$user->updateConfUser(array('main_logo' => 1), 'id IN (' . implode(',', secure::escQuoteData(array_keys($_POST['agencies']))) . ')');
			messages::messageChangeSaved(MESSAGE_CHANGE_SAVED, false, CONF_ADMIN_FILE . '?m=users&s=agencies');
		}
		// удаление с главной
		if ('remove' === $_POST['action'] && !empty($_POST['agencies'])) {
			$user->updateConfUser(array('main_logo' => 0), 'id IN (' . implode(',', secure::escQuoteData(array_keys($_POST['agencies']))) . ')');
			messages::messageChangeSaved(MESSAGE_CHANGE_SAVED, false, CONF_ADMIN_FILE . '?m=users&s=agencies');
		}
		// сортировка
	...
	$path = CONF_ADMIN_FILE . '?m=users&s=agencies&&amp;';
	...
	// формируем странциы
	$strPages = strings::generatePage($allRecords, $offset, CONF_AGENCIES_STRINGS_PERPAGE_ADMIN_PANEL, $path, true);
