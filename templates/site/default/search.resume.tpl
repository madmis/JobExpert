...
<form method="get" action="{$smarty.const.CONF_SCRIPT_URL}index.php?ut={$user_type}&amp;do=search.resume" enctype="application/x-www-form-urlencoded">
<input type="hidden" name="ut" value="{$user_type}">
<input type="hidden" name="do" value="search.resume">
						...
						<table width="100%" class="extSearchFieldTable">
							...
							<tr>
								<td>
									...
									{$smarty.const.SITE_FROM}&nbsp;<input style="text-align: right;" type="text" name="pay_from" size="4" maxlength="10" value="{$retFields.pay_from}">&nbsp;
									{$smarty.const.SITE_UNTO}&nbsp;<input style="text-align: right;" type="text" name="pay_post" size="4" maxlength="10" value="{$retFields.pay_post}">&nbsp;
							...
							<tr>
								<td>
									...
									<select name="education">
										...
										<option value="{if $key}{$item}{/if}" {if $retFields.education eq $item}selected{/if}>{$item}</option>
										{/foreach}
									</select>
								</td>
								<td>
							...
							<tr>
								<td>
									...
									<select name="user_type">
										...
										<option value="competitor" {if 'competitor' eq $arrBindFields.user_type}selected{/if}>{$smarty.const.FORM_TYPE_COMPETITOR}</option>
										<option value="agent" {if 'agent' eq $arrBindFields.user_type}selected{/if}>{$smarty.const.ANNOUNCE_RADIOBOX_USER_TYPE_AGENT}</option>
									</select>
								</td>
								<td>
									{* Выбор периода *}<br />
									...
									<select name="period">
										{foreach from=$arrSysDict.SearchPeriod.values item="item" key="key" name="period"}
...
{if $return_data}
	...
	{include file="resume.view.short.tpl"}
{/if}
