<form action="{$chpu->createChpuUrl("`$smarty.const.CONF_SCRIPT_URL`index.php?ut=$user_type&amp;do=user.news&amp;action=active")}" method="post" enctype="multipart/form-data">
		...
		{if $arrNewses}
			...
			{if $arrNews.link}
				<td class="AlignLeft" style="padding: 5px;" title="{$arrNews.title|escape}">
					<a href="{$chpu->createChpuUrl("`$smarty.const.CONF_SCRIPT_URL`index.php?ut=$user_type&amp;do=news&amp;action=view&amp;id=`$arrNews.tId`")}" target="_blank">
						{$arrNews.title|truncate}
					</a>
				</td>
			{else}
				<td class="AlignLeft" style="padding: 5px;" title="{$arrNews.title|escape}">
                		...
                		<tr>
                			<td>
								<select name="action" class="select">
									...
									{if $codex.rights.arc_news}<option value="archive">{$smarty.const.FORM_ACTION_ARCHIVE}</option>{/if}
