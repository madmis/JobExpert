...
{elseif $actPage.view}
	...
	$(document).ready(function() {
		...
		$('#respondResume').change(function() {
			if (!$(this).val()) {
				$('#viewRespondResume').hide();
			} else {
				$('#viewRespondResume').show();
			}
		});
		$('#viewRespondResume').click(function() {
			$.post('/ajax.php?getAnnounceData=vacancy', { unikey: $('#respondResume').val() }, function(result) {
				var response;
				try {
				...
				if (!response || !response.error) {
					response = $(document.createElement('div')).append(result).append(
						$(document.createElement('div')).addClass('DesignMainPageBody').append(
							$('#sendRespondResume').clone().click(function() {
								var tryCnt = 0;
								$.ajax({
									type: "POST",
									...
									data: {
										...
										email: $('#sendFrom').val(),
										subject: '{$smarty.const.ANNOUNCE_RESUME_RESPONSE_FORM_HEAD}: {$return_data.title}',
										text: result
									},
									success: function(msg){
										switch(msg) {
											...
											case 'success':
												...
												response.append($('#sendSuccessRespondResume').clone().show());
												$.fn.colorbox.resize();
												$('#viewRespondResume').hide();
												$('option[value="' + $('#respondResume').val() + '"]').remove();
												if ($('#respondResume > option').size() <= 1) {
													$('#trRespondResume').remove();
												}
												return;
											case 'errSendto':
											case 'errEmail':
