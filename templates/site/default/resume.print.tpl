...
{* ----------------------------------- *}
<hr style="color: #C1C1C1;" noshade="noshade" size="1">
{if $return_data.public_email || $return_data.visibility eq 'visible' || ($return_data.visibility eq 'members' && $user_email)}
	<p style="font-size: 16px; font-weight: bold;">{$smarty.const.ANNOUNCE_CONTACTS_HEAD}</p>
{/if}
{if $return_data.public_email}
    <table>
        <tr>
            <td>
                <strong>{$smarty.const.ANNOUNCE_CONTACTS_EMAIL}:</strong>&nbsp;<a href="mailto:{$return_data.email}" class="mailto" title="{$smarty.const.FORM_SEND_EMAIL}" rel="nofollow">{$return_data.email}</a>
            </td>
        </tr>
    </table>
{/if}
