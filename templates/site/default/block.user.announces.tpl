{if $menu eq 'user.announces'}
	{if $action eq 'vacancy' && ($user_type eq 'employer' || $user_type eq 'company' || $user_type eq 'agent')}
		{counter start=0 print=false}
		<div class="DesignLeftSideBarBlockWrapper">
			<h3 class="sideBlockHeader" id="vacancy">{$smarty.const.MENU_MY_VACANCYS}</h3>
			<ul class="sideBlockContent">
				<li class="{if ({counter}%2)}even{else}odd{/if}">
					{if $token eq 'active'}
						<a class="active">{$smarty.const.ANNOUNCE_TOKEN_ACTIVE} {$smarty.const.FORM_VACANCYS_HEAD}</a>
					{else}
						<a href="{$chpu->createChpuUrl("`$smarty.const.CONF_SCRIPT_URL`index.php?ut=$user_type&amp;do=$menu&amp;action=$action&amp;token=active")}">{$smarty.const.ANNOUNCE_TOKEN_ACTIVE} {$smarty.const.FORM_VACANCYS_HEAD}</a>
					{/if}
				</li>
				<li class="{if ({counter}%2)}even{else}odd{/if}">
	...
	{/if}
	{if $action eq 'resume' && ($user_type eq 'competitor' || $user_type eq 'agent')}
		{counter start=0 print=false}
		<div class="DesignLeftSideBarBlockWrapper">
