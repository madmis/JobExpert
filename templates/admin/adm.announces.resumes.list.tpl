<table class="table_announce">
		...
		<tr>
			<td colspan="2">
				...
				<div class="comments_correction">
					<p class="p_5">{$resume.comments|nl2br}</p>
				</div>
			</td>
		</tr>
	{/if}
	<tr>
        ...
        {/if}
		<td style="width: 70%;">
			<p class="p_head" style="text-align: left; padding-bottom: 5px;">{$resume.title}</p>
			<p class="p_name">
		...
		<td style="width: 30%;" class="short_info">
			<div class="ext_subinfo">{$smarty.const.ANNOUNCE_SELECT_ACTPERIOD}:&nbsp;<span style="font-weight: bold;">{$act_period[$resume.act_period]}</span></div>
			...
			<p class="p_5">{$smarty.const.ANNOUNCE_PAY_HEAD}:&nbsp;{$smarty.const.SITE_FROM}&nbsp;<span style="font-weight: bold;">{$resume.pay_from}&nbsp;{$resume.currency}</span></p>
	...
	{if $resume.educations}
		<tr>
			<td colspan="2" valign="top" class="ext_info">
