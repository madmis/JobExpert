{*Резюме ожидающие модерации*}
<form action="{$smarty.const.CONF_ADMIN_FILE}?m=announces&amp;s=resumes&amp;action=moderate" method="post" enctype="multipart/form-data">
	<table style="width: 100%; border: 0px;" cellspacing="5" cellpadding="3">
		<thead class="data_head">
			<tr>
				{if $return_data}
					<td style="width: 65%;">
						<a href="{$smarty.const.CONF_ADMIN_FILE}?m=announces&amp;s=resumes&amp;action=moderate&amp;order=title&amp;by={if $order.title eq 'ASC'}DESC{else}ASC{/if}" class="white" title="{if $order.title eq 'ASC'}{$smarty.const.ANNOUNCE_SORT_DOWN}{else}{$smarty.const.ANNOUNCE_SORT_UP}{/if}">
							{$smarty.const.FORM_RESUMES_HEAD}:&nbsp;{$smarty.const.MENU_ACTION_MODERATE}
					...
					<td style="width: 30%;">
						<a href="{$smarty.const.CONF_ADMIN_FILE}?m=announces&amp;s=resumes&amp;action=moderate&amp;order=token_datetime&amp;by={if $order.token_datetime eq 'ASC'}DESC{else}ASC{/if}" class="white" title="{if $order.token_datetime eq 'ASC'}{$smarty.const.ANNOUNCE_SORT_DOWN}{else}{$smarty.const.ANNOUNCE_SORT_UP}{/if}">
				...
				{else}
					<td style="width: 65%;">{$smarty.const.FORM_RESUMES_HEAD}:&nbsp;{$smarty.const.MENU_ACTION_MODERATE}</td>
			...
			<div id="openres_{$resume.id}">
				...
				<form action="{$smarty.const.CONF_ADMIN_FILE}?m=announces&amp;s=resumes&amp;action=moderate" method="post" enctype="multipart/form-data">
					...
					<div style="display: none;">
						...
						<div class="p_0"><textarea name="arrResData[comments]" class="comments" cols="100" rows="5" style="border: 1px solid #CC3333;"></textarea></div>
					</div>
