<form id="AddForm" action="{$smarty.const.CONF_ADMIN_FILE}?m=manager&amp;s=dop.pages&amp;action=add" method="post" enctype="multipart/form-data">
<table style="width: 100%; border: 0px; text-align: left;">
	...
	<tr>
		<td colspan="2">
			...
			<input type="text" name="arrBindFields[title]" value="{$return_data.title}" size="100" class="text">
		</td>
	</tr>
	<tr>
		<td colspan="2">
			<p>{$smarty.const.FORM_DOP_PAGE_TEXT}</p>
			<textarea name="arrNoBindFields[text]" id="area_text" rows="20" cols="100" {if $smarty.const.CONF_USE_VISUAL_EDITOR}class="tinymce"{else}class="text"{/if}>{$return_data.text}</textarea>
		</td>
	</tr>
	<tr>
		<td colspan="2">
	...
	<tr>
		<td colspan="2">
			{$smarty.const.TABLE_COLUMN_SORT} <input type="text" name="arrNoBindFields[sort]" value="{if $return_data.sort}{$return_data.sort}{else}0{/if}" size="5" class="text">
