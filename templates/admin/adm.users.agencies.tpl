...
{* Настройки агентств *}
{if $action.config}
	{include file="adm.users.agencies.config.tpl"}
{else if $action.seo}
	{include file="adm.manager.seo.tpl"}
{* Список агентств *}
{else}
	<form action="{$smarty.const.CONF_ADMIN_FILE}?m=users&amp;s=agencies" method="post" enctype="multipart/form-data">
	...
	{if $arrCompanies}
		{foreach from=$arrCompanies item="company" name="i"}
			<tr>
				...
				<td style="text-align: center;">{if !$company.logo}-{else}<input type="checkbox" name="agencies[{$company.id}]" class="checkbox_entry">{/if}</td>
			</tr>
		{/foreach}
		</tbody>
