{*Резюме ожидающие оплаты*}
<form action="{$smarty.const.CONF_ADMIN_FILE}?m=announces&amp;s=resumes&amp;action=payment" method="post" enctype="multipart/form-data">
	<table style="width: 100%; border: 0px;" cellspacing="5" cellpadding="3">
		<thead class="data_head">
			<tr>
				{if $return_data}
					<td style="width: 65%;">
						<a href="{$smarty.const.CONF_ADMIN_FILE}?m=announces&amp;s=resumes&amp;action=payment&amp;order=title&amp;by={if $order.title eq 'ASC'}DESC{else}ASC{/if}" class="white" title="{if $order.title eq 'ASC'}{$smarty.const.ANNOUNCE_SORT_DOWN}{else}{$smarty.const.ANNOUNCE_SORT_UP}{/if}">
							{$smarty.const.FORM_RESUMES_HEAD}:&nbsp;{$smarty.const.MENU_ACTION_PAYMENT}
					...
					<td style="width: 30%;">
						<a href="{$smarty.const.CONF_ADMIN_FILE}?m=announces&amp;s=resumes&amp;action=payment&amp;order=token_datetime&amp;by={if $order.token_datetime eq 'ASC'}DESC{else}ASC{/if}" class="white" title="{if $order.token_datetime eq 'ASC'}{$smarty.const.ANNOUNCE_SORT_DOWN}{else}{$smarty.const.ANNOUNCE_SORT_UP}{/if}">
				...
				{else}
					<td style="width: 65%;">{$smarty.const.FORM_RESUMES_HEAD}:&nbsp;{$smarty.const.MENU_ACTION_PAYMENT}</td>
		...
		<tbody class="data_body">
			{foreach from=$return_data item="resume" name="resume"}
				<tr class="tr_hover">
					...
					<td style="text-align: center; width: 5%;"><input type="checkbox" name="arrResData[id][{$resume.id}]" class="checkbox_entry"></td>
				</tr>
			{/foreach}
		</tbody>
			...
			<div id="openres_{$resume.id}">
				...
				<form action="{$smarty.const.CONF_ADMIN_FILE}?m=announces&amp;s=resumes&amp;action=payment" method="post" enctype="multipart/form-data">
					<p>
						...
						<input type="radio" name="arrResData[action]" value="active" checked="checked"> {$smarty.const.FORM_ACTION_ACTIVATE}
	...
	$('.announce_detail').click(function() {
		...
		$(targ).find('form').submit(function() {
			...
			if (!action) {
				...
				return false;
			} else if ('active' !== action && !$(this).find('.comments').val()) {
