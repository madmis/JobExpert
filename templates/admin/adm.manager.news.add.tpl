<form action="{$smarty.const.CONF_ADMIN_FILE}?m=manager&amp;s=news&amp;action=add" method="post" enctype="multipart/form-data">
<table style="width: 100%; border: 0px; text-align: left;">
	<tr>
		<td>
			...
			<label><input type="checkbox" name="arrBindFields[token]" {if $retFields.token eq 'active'}checked="checked"{/if}>&nbsp;{$smarty.const.RECORD_ACTIVE}</label>
