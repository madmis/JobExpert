		...
		<tr>
			<td>
				...
				{foreach from=$arrRecords item="record"}
					...
					{else}
						<a href="{$smarty.const.CONF_ADMIN_FILE}?m=manager&amp;s=articles&amp;action=comments{$strFilter}{$strSort}&amp;records={$record}" class="white">
							{$record}
						</a>&nbsp;
					{/if}
				{/foreach}
				...
				<table style="width: 100%; border-spacing: 10px;">
					<tr>
						<td style="width: 50%; border: 0px; vertical-align: top;">
							...
							{/if}
							<a href="{$smarty.const.CONF_ADMIN_FILE}?m=manager&amp;s=articles&amp;action=comments{$smarty.capture.filter}{$strSort}"><img src="{$smarty.const.TEMPLATE_PATH_ADMIN}images/icons/{$smarty.capture.image}" alt="{$smarty.capture.const}" title="{$smarty.capture.const}"></a>
					...
					<tr style="height: 10px;">
						...
						<td style="width: 50%; border: 0px; vertical-align: top;">
							{if !$arrSort || ($arrSort.order eq 'datetime' && $arrSort.by eq 'DESC')}
								<a href="{$smarty.const.CONF_ADMIN_FILE}?m=manager&amp;s=articles&amp;action=comments{$strFilter}&amp;order=datetime&amp;by=ASC"><img src="{$smarty.const.TEMPLATE_PATH_ADMIN}images/icons/sort_down.gif" alt="{$smarty.const.ANNOUNCE_SORT_UP}" title="{$smarty.const.ANNOUNCE_SORT_UP}"></a>
							{else}
								<a href="{$smarty.const.CONF_ADMIN_FILE}?m=manager&amp;s=articles&amp;action=comments{$strFilter}&amp;order=datetime&amp;by=DESC"><img src="{$smarty.const.TEMPLATE_PATH_ADMIN}images/icons/sort_up.gif" alt="{$smarty.const.ANNOUNCE_SORT_DOWN}" title="{$smarty.const.ANNOUNCE_SORT_DOWN}"></a>
							{/if}
					...
					<tr>
						<td style="width: 50%; border: 0px; vertical-align: top;">
							{if $arrFilter.filter eq 'id_article'}
								{capture name='filter'}{/capture}
								{capture name='const'}{$smarty.const.ANNOUNCE_FILTER_OFF}{/capture}
								{capture name='image'}unfilter.png{/capture}
							{else}
								{capture name='filter'}&amp;filter=id_article&amp;in={$comment.id_article}{/capture}
								{capture name='const'}{$smarty.const.ANNOUNCE_FILTER_ON}{/capture}
								{capture name='image'}filter.png{/capture}
							{/if}
							<a href="{$smarty.const.CONF_ADMIN_FILE}?m=manager&amp;s=articles&amp;action=comments{$smarty.capture.filter}{$strSort}"><img src="{$smarty.const.TEMPLATE_PATH_ADMIN}images/icons/{$smarty.capture.image}" alt="{$smarty.capture.const}" title="{$smarty.capture.const}"></a>
							<span class="detail">{$comment.title}</span>
							<input type="hidden" value="{$comment.id_article}">
						</td>
					</tr>
				</table>
			</td>
