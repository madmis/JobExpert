{*Вакансии ожидающие активации по email*}
<form action="{$smarty.const.CONF_ADMIN_FILE}?m=announces&amp;s=vacancys&amp;action=new" method="post" enctype="multipart/form-data">
	<table style="width: 100%; border: 0px;" cellspacing="5" cellpadding="3">
		<thead class="data_head">
			<tr>
				{if $return_data}
					<td style="width: 65%;">
						<a href="{$smarty.const.CONF_ADMIN_FILE}?m=announces&amp;s=vacancys&amp;action=new&amp;order=title&amp;by={if $order.title eq 'ASC'}DESC{else}ASC{/if}" class="white" title="{if $order.title eq 'ASC'}{$smarty.const.ANNOUNCE_SORT_DOWN}{else}{$smarty.const.ANNOUNCE_SORT_UP}{/if}">
							{$smarty.const.FORM_VACANCYS_HEAD}:&nbsp;{$smarty.const.MENU_ACTION_NEW}
					...
					<td style="width: 30%;">
						<a href="{$smarty.const.CONF_ADMIN_FILE}?m=announces&amp;s=vacancys&amp;action=new&amp;order=token_datetime&amp;by={if $order.token_datetime eq 'ASC'}DESC{else}ASC{/if}" class="white" title="{if $order.token_datetime eq 'ASC'}{$smarty.const.ANNOUNCE_SORT_DOWN}{else}{$smarty.const.ANNOUNCE_SORT_UP}{/if}">
				...
				{else}
					<td style="width: 65%;">{$smarty.const.FORM_VACANCYS_HEAD}:&nbsp;{$smarty.const.MENU_ACTION_NEW}</td>
