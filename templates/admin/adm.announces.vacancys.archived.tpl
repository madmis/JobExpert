{*Вакансии в архиве*}
<form action="{$smarty.const.CONF_ADMIN_FILE}?m=announces&amp;s=vacancys&amp;action=archived" method="post" enctype="multipart/form-data">
	<table style="width: 100%; border: 0px;" cellspacing="5" cellpadding="3">
		<thead class="data_head">
			<tr>
				{if $return_data}
					<td style="width: 65%;">
						<a href="{$smarty.const.CONF_ADMIN_FILE}?m=announces&amp;s=vacancys&amp;action=archived&amp;order=title&amp;by={if $order.title eq 'ASC'}DESC{else}ASC{/if}" class="white" title="{if $order.title eq 'ASC'}{$smarty.const.ANNOUNCE_SORT_DOWN}{else}{$smarty.const.ANNOUNCE_SORT_UP}{/if}">
							{$smarty.const.FORM_VACANCYS_HEAD}:&nbsp;{$smarty.const.ANNOUNCE_TOKEN_ARCHIVED}
					...
					<td style="width: 30%;">
						<a href="{$smarty.const.CONF_ADMIN_FILE}?m=announces&amp;s=vacancys&amp;action=archived&amp;order=token_datetime&amp;by={if $order.token_datetime eq 'ASC'}DESC{else}ASC{/if}" class="white" title="{if $order.token_datetime eq 'ASC'}{$smarty.const.ANNOUNCE_SORT_DOWN}{else}{$smarty.const.ANNOUNCE_SORT_UP}{/if}">
							{$smarty.const.FORM_ARCHIVED}
				...
				{else}
					<td style="width: 65%;">{$smarty.const.FORM_VACANCYS_HEAD}:&nbsp;{$smarty.const.ANNOUNCE_TOKEN_ARCHIVED}</td>
					<td style="width: 30%;">{$smarty.const.FORM_ARCHIVED}</td>
			...
			<tr>
				...
				<td colspan="3" style="text-align: right;">
					<select name="arrVacData[action]" class="select">
						...
						<option value="active">{$smarty.const.FORM_ACTION_ACTIVATE}</option>
			...
			<div id="openvac_{$vacancy.id}">
				...
				<form action="{$smarty.const.CONF_ADMIN_FILE}?m=announces&amp;s=vacancys&amp;action=archived" method="post" enctype="multipart/form-data">
					<p>
						...
						<input type="radio" name="arrVacData[action]" value="active">&nbsp;{$smarty.const.FORM_ACTION_ACTIVATE}
