...
<div id="addConstLang_custom" title="{$smarty.const.FORM_BUTTON_ADD_LANG_CONST_CUSTOM}" style="display: none;">
	...
	<form action="#" id="fAddConstLang_custom">
		<fieldset class="ui-corner-all">
			...
			<b>{$modId|upper}_CONST_CUSTOM_</b><input type="text" name="nameConst" id="nameConst" size="55" maxlength="255" class="text ui-widget-content ui-corner-all">
			<br><br>
		...
		buttons: {
			'{$smarty.const.FORM_BUTTON_ADD}': function() {
				if (!$('#nameConst').val() ) {
					...
					$.post(
						'/admajax.php?mods=payments&modId={$modId}&do=addConstLang_custom',
	...
	$('#delConstLang_custom').dialog({
		...
		buttons: {
			'{$smarty.const.FORM_BUTTON_DELETE}': function() {
				var thisDialog = $(this);
                $.post(
                    '/admajax.php?mods=payments&modId={$modId}&do=delConstLang_custom',
